...
class MazeGame:
    def __init__(self, root, cols=21, rows=15, cell_size=30, time_limit_sec=30):
        self.root = root
        ...
        ttk.Button(btn_frame, text="Играть", command=self.start_dfs,
                   style="Accent.TButton").pack(side=tk.LEFT, padx=6)

        self.timer_label = tk.Label(root, text="Время: -- с",
                                    font=("Segoe UI", 12, "bold"), bg="#f4efe6")
        self.timer_label.pack()
        self.hint_label = tk.Label(root, text="Нажмите кнопку - Играть, чтобы начать побег",
    ...
    def _setup_styles(self):
        ...
        self.exit_font = ("Segoe UI", 11, "bold")
    ...
    def generate_maze(self):
        ...
        if len(passages) >= 2:
            self.exit_pos = random.choice(passages)
    ...
    def draw_maze(self):
        ...
        self.canvas.create_text(cx, cy, text="Выход", fill="#c62828", font=self.exit_font)
...
if __name__ == "__main__":
    ...
    game = MazeGame(root, cols=21, rows=15, cell_size=30, time_limit_sec=30)
    root.mainloop()
